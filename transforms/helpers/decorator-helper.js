...
function createClassDecorator(j, classDecoratorProp) {
    ...
    return j.decorator(j.callExpression(j.identifier(classDecoratorProp.classDecoratorName), [...decoratorArgs]));
}
/**
...
function createCallExpressionDecorators(j, decoratorName, instanceProp) {
    ...
    let decoratorExpression = ['computed', 'service', 'controller'].includes(decoratorName) && decoratorArgs.length === 0
        ? j.identifier(decoratorName)
        : j.callExpression(j.identifier(decoratorName), decoratorArgs);
    decoratorExpression = instanceProp.modifiers.reduce((callExpr, modifier) => j.callExpression(j.memberExpression(callExpr, modifier.prop), modifier.args), decoratorExpression);
...
function createDecoratorsWithArgs(j, identifier, args) {
    return [
        j.decorator(j.callExpression(j.identifier(identifier), args.map((arg) => j.literal(arg)))),
    ];
}
/**
...
function createInstancePropDecorators(j, instanceProp) {
    return instanceProp.decoratorNames.reduce((decorators, decorator) => {
        ...
        if (decorator === 'off' || decorator === 'unobserves') {
            return decorators.concat(createDecoratorsWithArgs(j, decorator, instanceProp.decoratorArgs[decorator]));
        }

"use strict";
const { DECORATOR_PATHS, DECORATOR_PATH_OVERRIDES, EMBER_DECORATOR_SPECIFIERS, get, getFirstDeclaration, METHOD_DECORATORS, } = require('./util');
 ...
 */
function createNewImportDeclarations(j, root, decoratorsToImport, decoratorPathsToIgnore = [], options = {}) {
    const firstDeclaration = getFirstDeclaration(j, root);
    if (options.classicDecorator) {
        firstDeclaration.insertBefore(createImportDeclaration(j, [j.importDefaultSpecifier(j.identifier('classic'))], 'ember-classic-decorator'));
    }
    ...
    Object.keys(EMBER_DECORATOR_SPECIFIERS)
        ...
        .forEach((path) => {
        const specifiers = createEmberDecoratorSpecifiers(j, EMBER_DECORATOR_SPECIFIERS[path], decoratorsToImport);
...
function getDecoratorPathSpecifiers(j, root, decoratorsToImport = []) {
    ...
    return getExistingDecoratorImports(j, root).reduce((decoratorPathSpecifierMap, existingDecoratorImport) => {
        const { importPropDecoratorMap, decoratorPath } = DECORATOR_PATHS[get(existingDecoratorImport, 'value.source.value')];
        ...
        const decoratedSpecifiers = createEmberDecoratorSpecifiers(j, decoratorsForPath, decoratorsToImport);
        ...
        for (let i = existingSpecifiers.length - 1; i >= 0; i -= 1) {
            ...
            if (isSpecifierDecorator(existingSpecifier, importPropDecoratorMap)) {
                ...
                if (overriddenPath) {
                    decoratorPathSpecifierMap[overriddenPath] = [].concat(decoratorPathSpecifierMap[overriddenPath] || [], existingSpecifier);
                }
                else {
                    const isSpecifierPresent = decoratedSpecifiers.some((specifier) => {
                        return (!get(specifier, 'local.name') &&
        ...
        if (decoratedSpecifiers.length) {
            decoratorPathSpecifierMap[decoratorPath] = [].concat(decoratorPathSpecifierMap[decoratorPath] || [], decoratedSpecifiers);
    ...
    return getExistingDecoratorImports(j, root).reduce((importedDecorators, decoratorImport) => {
        ...
        return specifiers.reduce((importedDecorators, specifier) => {
            if (isSpecifierDecorator(specifier, importPropDecoratorMap)) {
                importedDecorators[get(specifier, 'local.name')] = getDecoratorInfo(specifier, importPropDecoratorMap);
            }

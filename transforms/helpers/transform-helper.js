"use strict";
const { ACTION_SUPER_EXPRESSION_COMMENT, LAYOUT_DECORATOR_LOCAL_NAME, LAYOUT_DECORATOR_NAME, get, getPropName, shouldSetValue, } = require('./util');
const { withDecorators, createClassDecorator, createInstancePropDecorators, createIdentifierDecorators, } = require('./decorator-helper');
/**
...
function instancePropsToExpressions(j, instanceProps) {
    return instanceProps.map((instanceProp) => withComments(j.expressionStatement(j.assignmentExpression('=', j.memberExpression(j.thisExpression(), instanceProp.key), instanceProp.value)), instanceProp));
}
/**
    ...
    superExprs.forEach((superExpr) => {
        if (replaceWithUndefined) {
            ...
            if (functionProp.isComputed) {
                ...
                superMethodCall = j.callExpression(j.memberExpression(j.memberExpression(j.memberExpression(j.super(), j.identifier('actions')), methodDefinition.key), j.identifier('call')), [].concat(j.thisExpression(), ...superMethodArgs));
                superMethodCall.comments = createLineComments(j, ACTION_SUPER_EXPRESSION_COMMENT);
            }
            else {
                superMethodCall = j.callExpression(j.memberExpression(j.super(), methodDefinition.key), superMethodArgs);
            }
...
function createMethodProp(j, functionProp, decorators = []) {
    ...
    return withDecorators(withComments(replaceSuperExpressions(j, j.methodDefinition(propKind, functionProp.key, functionProp.value), functionProp), functionProp), decorators);
}
/**
...
function createConstructor(j, instanceProps = []) {
    if (instanceProps.length) {
        return [
            j.methodDefinition('constructor', j.identifier('constructor'), j.functionExpression(null, [], j.blockStatement([createSuperExpressionStatement(j)].concat(instancePropsToExpressions(j, instanceProps))))),
        ];
    }
    return [];
}
...
function createClassProp(j, instanceProp, decorators = []) {
    ...
    const classProp = withDecorators(withComments(j.classProperty(instanceProp.key, shouldSetValue(instanceProp) ? instanceProp.value : null, null), instanceProp), decorators);
...
function convertIdentifierActionToMethod(j, idAction, decorators = []) {
    const returnBlock = j.blockStatement([
        j.returnStatement(j.callExpression(j.memberExpression(idAction.value, j.identifier('call')), [
            j.thisExpression(),
            j.spreadElement(j.identifier('arguments')),
        ])),
    ]);
    const expr = j.functionExpression(null, [], returnBlock);
    return withDecorators(withComments(j.methodDefinition('method', idAction.key, expr), idAction), decorators);
}
/**
...
function createCallExpressionProp(j, callExprProp) {
    ...
    if (callExprProp.shouldRemoveLastArg) {
        if (lastArgType === 'FunctionExpression') {
            ...
            withDecorators(withComments(callExprMethods[0], callExprProp), createInstancePropDecorators(j, callExprProp));
...
function createSuperClassExpression(j, superClassName = '', mixins = []) {
    if (mixins.length > 0) {
        return j.callExpression(j.memberExpression(j.identifier(superClassName), j.identifier('extend')), mixins);
    }
 ...
 */
function createClass(j, className, { instanceProps = [] } = {}, superClassName = '', mixins = [], options) {
    let classBody = [];
    let classDecorators = [];
    ...
    return withDecorators(j.classDeclaration(className ? j.identifier(className) : null, j.classBody(classBody), createSuperClassExpression(j, superClassName, mixins)), classDecorators);
}
/**
...
function createEmberDecoratorSpecifiers(j, pathSpecifiers = [], decoratorsToImport = []) {
    return pathSpecifiers
        ...
        .map((specifier) => {
        const importedSpecifier = specifier === LAYOUT_DECORATOR_LOCAL_NAME ? LAYOUT_DECORATOR_NAME : specifier;

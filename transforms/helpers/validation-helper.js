 ...
 * @returns {Boolean}
 */
function hasValidProps(j, { instanceProps = [] } = {}, { decorators = true, classFields = true } = {}) {
    ...
    return instanceProps.reduce((errors, instanceProp) => {
        ...
        if (instanceProp.type === 'ObjectExpression' &&
            !['actions', 'queryParams'].includes(instanceProp.name)) {
            errors.push(`[${instanceProp.name}]: Transform not supported - value is of type object. For more details: eslint-plugin-ember/avoid-leaking-state-in-ember-objects`);
        }
        ...
        if ((!decorators && (instanceProp.hasDecorators || instanceProp.isClassDecorator)) ||
            unsupportedPropNames.includes(instanceProp.name) ||
            (instanceProp.isCallExpression && !instanceProp.hasDecorators)) {
            errors.push(`[${instanceProp.name}]: Transform not supported - need option '--decorators=true' or the property type ${instanceProp.type} can not be transformed`);
        }
        if (instanceProp.hasModifierWithArgs) {
            errors.push(`[${instanceProp.name}]: Transform not supported - value has modifiers like 'property' or 'meta'`);
        }
        if (instanceProp.hasVolatile && instanceProp.hasMetaDecorator) {
            errors.push(`[${instanceProp.name}]: Transform not supported - value has 'volatile' modifier with computed meta ('@ember/object/computed') is not supported`);
        }
        return errors;
    }, []);
}
...
function getLifecycleHookErrors(actionsProp) {
    ...
    return actionProps.reduce((errors, actionProp) => {
        ...
        if (actionName && LIFECYCLE_HOOKS.includes(actionName)) {
            errors.push(`[${actionName}]: Transform not supported - action name matches one of the lifecycle hooks. Rename and try again. See https://github.com/scalvert/ember-native-class-codemod/issues/34 for more details`);
        }
        return errors;
    }, []);
}
    ...
    return actionProps.reduce((errors, actionProp) => {
        ...
        if (actionName) {
            ...
            if (actionLiterals.length || actionCalls.length) {
                errors.push(`[${actionName}]: Transform not supported - calling the passed action would cause an infinite loop. See https://github.com/scalvert/eslint-plugin-ember-es6-class/pull/2 for more details`);
            }
        }
        return errors;
    }, []);
